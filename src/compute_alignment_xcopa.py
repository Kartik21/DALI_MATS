...
def compute_alignment(args):
    ...
    with open(os.path.join(args.embedding_path, f"english_{args.lang}_choice1.pkl"), "rb") as pickle_file:
        english_choice1 = pickle.load(pickle_file)

    with open(os.path.join(args.embedding_path, f"english_{args.lang}_choice2.pkl"), "rb") as pickle_file:
    ...
    for layer in range(32):
        for idx in range(1511):
            ...
            if (cs_11_lasttoken > cs_12_lasttoken) & (cs_22_lasttoken > cs_21_lasttoken) & (cs_11_lasttoken > cs_21_lasttoken) & (cs_22_lasttoken > cs_12_lasttoken):
                binary_alignment_matrix_lasttoken[idx][layer] = 1
            else:
                binary_alignment_matrix_lasttoken[idx][layer] = 0

            if (cs_11_weighted > cs_12_weighted) & (cs_22_weighted > cs_21_weighted) & (cs_11_weighted > cs_21_weighted) & (cs_22_weighted > cs_12_weighted):
...
if __name__ == '__main__':
     ...
     parser.add_argument('--save_path', type=str, default='../alignment_outputs/Llama3.1/xcopa_dali/')
     parser.add_argument('--embedding_path', type=str,default='/fs/nexus-scratch/kravisan/embeddings/Llama3.1/xcopa/')
